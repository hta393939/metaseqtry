...
MQHandleInfo::MQHandleInfo(MQScene scene, const MQPoint& center_pos, bool check_visibility)
{
	...
	MQPoint r = center_pos;
	MQPoint p, q;
	p = scene->Convert3DToScreen(r);
	if(p != MQPoint(0,0,0)){
		q = scene->ConvertScreenTo3D(MQPoint(p.x+70.0f,p.y+70.0f,p.z));
	}else{
		r = scene->GetRotationCenter(); 
		p = scene->Convert3DToScreen(r);
		q = scene->ConvertScreenTo3D(MQPoint(p.x+70.0f,p.y+70.0f,p.z));
	}
...
void MQHandleOperation::Reposition(POINT mousePos)
{
	DownPos = mousePos;
	PrevPos = mousePos;
	TotalAngle = 0;
